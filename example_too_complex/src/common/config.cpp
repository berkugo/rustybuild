...
namespace config_heavy {
    template<int N> struct Tri { static const int value = N + Tri<N-1>::value; };
    template<> struct Tri<0> { static const int value = 0; };
    template<typename T, int D> struct Vec { T v[D]; };
}
void config_compile_heavy_anchor() {
    (void)config_heavy::Tri<64>::value;
    config_heavy::Vec<double,16> v{};
    (void)v.v[0];
}
