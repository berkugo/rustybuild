...
namespace vec_heavy {
    template<int N> struct Hex { static const int value = N * 16 + Hex<N-1>::value; };
    template<> struct Hex<0> { static const int value = 0; };
    template<int N> int mul_chain(int x) { return x * mul_chain<N-1>(x); }
    template<> int mul_chain<0>(int) { return 1; }
}
void vector_compile_heavy_anchor() {
    (void)vec_heavy::Hex<8>::value;
    (void)vec_heavy::mul_chain<10>(2);
}
